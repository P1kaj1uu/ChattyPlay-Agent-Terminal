...
</style></head><body><main><header><div><h1>ChattyPlay Control Room</h1><p>本地模型、权限、Skills、MCP、RAG 与 LLM Wiki</p></div><span class="badge">LOCAL · 127.0.0.1</span></header>
...
<section class="card wide"><h2>LLM Wiki 项目知识库</h2><p id="wikistatus" class="status"></p><p>在终端运行 <code>/wiki build</code> 编译或更新，<code>/wiki show</code> 查看内容。</p></section>
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if path == "/api/state":
                ...
                self._json(200, {"config": config, "path": str(store.project_path), "ollama": ollama_status(), "recommended_model": CHAT_MODEL, "rag": RAGIndex(store.workspace, config.get("rag", {})).status(), "wiki": ProjectWiki(store.workspace).status(), "skills": [
